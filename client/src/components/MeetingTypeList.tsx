...
const apiURI = 'http://192.168.100.9:8080'; // flask address
...
const MeetingTypeList = () => {
	...
	const createMeeting = async (): Promise<void> => {
		...
		try {
			...
			const response = await axios.post(`/api/create_room`, data, {
				headers: {
			...
			localStorage.setItem('role', 'create');
			localStorage.setItem('user_id', String(1));
	...
	const joinMeeting = async () => {
		try {
			...
			const _ = await axios.post(`/api/join_room`, data, {
				headers: {
					'Content-Type': 'application/json',
					Authorization: 'Bearer YOUR_TOKEN_HERE', // FIX IT LATER!
				},
			...
			localStorage.setItem('user_id', String(2));
