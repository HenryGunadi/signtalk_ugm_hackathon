...
function createPeerConnection() {
    ...
    // public google stun server, note if stun server fails, try TURN server
    ...
    // create peer connection / pc
    pc = new RTCPeerConnection(config);

    // some listeners
    // we could skip this its just for debugging
...
// calculate total of input device
function enumerateInputDevice() {
...
function negotiate() {
    return pc
        .createOffer() // <-- createOffer returns a promise (async)
        .then((offer) => {
        ...
        .then(() => {
            ...
            return new Promise((resolve) => {
                if (pc.iceGatheringState === "complete") {
                    ...
                    pc.addEventListener("icegathetingstatechange", checkState);
                }
            });
        })
        .then(() => {
            ...
            // for customization only, sdp is created automatically when creating an offer
        ...
        navigator.mediaDevices.getUserMedia(constraints).then(
            (stream) => {
                stream.getTracks().forEach((track) => {
                    pc.addTrack(track, stream); // <-- add track to connection
                });
